...
> **Status (2026-04-21)**: **v1.2 paper2any shipped + v1.2.5 polish + v1.3.0/v1.3.1 interactive academic landings**. Full 3-artifact coverage (poster + landing + deck) × full paper → artifact pipeline (now also eating `.docx` / `.pptx` / scanned-PDF), and landings now emit with CTAs, auto-nav, reveal-on-scroll, and semantic `<header>/<main>/<footer>`. Paper landings pull **the paper's own figures + tables** (not NBP stock icons) — verified end-to-end on longcat-next (**0 NBP calls, 9 paper figures + 1 benchmark table across 8 sections**, pass 0.88 in one iteration). 11 tools wired, smoke 18/18 green.
>
...
> - **v1.3.1** — paper-landing imagery policy: prompt-only fix that routes `ingest_fig_NN` / `ingest_table_NN` into content sections (highlights / method / results / showcase) and reserves NBP for hero-only. Default style `editorial` (explicitly discourages claymorphism / neubrutalism / glassmorphism for papers). Re-dogfood on longcat-next flipped from 5 NBP icons + 1 paper figure (marketing vibe) to **0 NBP + 9 paper figures + 1 benchmark table** (academic) in one planner iteration.
>
