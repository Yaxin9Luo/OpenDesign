...
│                         + caption matching + fake-figure filtering) / MD /   │
│                         image → structured manifest; registers kind="image"  │
│                         figures + kind="table" layers with rows/headers/     │
│                         col_highlight_rule in rendered_layers for planner    │
...
| 2 | `ingest_document` | [`tools/ingest_document.py`](../longcat_design/tools/ingest_document.py) | v1.2 paper2any. Dispatches on extension: `.pdf` → pymupdf native raster/vector figure extraction + `find_tables()` localization; Qwen-VL-Max (via `util/vlm.py`) handles structure manifest + caption matching + fake-figure filtering + table cell parsing. `.md,.txt` → embedded `![](…)` ref resolution. `.png/.jpg/.jpeg/.webp` → passthrough. Registers `ingest_fig_NN` (`kind="image"`) + `ingest_table_NN` (`kind="table"` with `rows`, `headers`, `col_highlight_rule`) in `rendered_layers`. Tool result summarizes the top-20 figure catalog for planner picks. | Writes `layers/img_<id>.png`; registers records in `rendered_layers`; stores manifest in `ctx.state["ingested"]`. |
