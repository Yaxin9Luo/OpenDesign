...
> **Status (2026-04-19): 7 of 11 items shipped.** Rename ✅, #3 switch_artifact_type ✅, #4 chat shell ✅, #5 edit_layer ✅, #6 HTML renderer + edit toolbar ✅, **#6.5 `apply-edits` CLI ✅ — round-trip edited HTML → new PSD/SVG/HTML/PNG with `metadata.parent_run_id` lineage**. Landing-mode HTML is part of #8. Next: #7 PPTX renderer (deck output) or #8 landing schema.
...
**Effort done**: ~13.5 h (items 1-6.5, both #6 and #6.5 came in under budget). **Remaining**: ~7 h coding (items 7 + 8 + 11) + ~4 h docs/video (items 9, 10).
