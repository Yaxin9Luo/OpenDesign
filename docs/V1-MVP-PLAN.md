...
> **Status (2026-04-19): 6 of 11 items shipped** (7 if you count #6.5 apply-edits as separate). Rename ✅, #3 switch_artifact_type ✅, #4 chat shell ✅, #5 edit_layer ✅, **#6 HTML renderer (poster mode) ✅ — with in-browser edit toolbar (drag + font/size/color) + Tier 1 Save (Copy / Download edited HTML)**. Landing-mode HTML is part of #8. Next: #6.5 `apply-edits` CLI (round-trip edited HTML → PSD/SVG/PNG).
...
| 6.5 | **`apply-edits` CLI — round-trip edited HTML → PSD/SVG/PNG** | 2 h | pending ← **NEXT** | New `apply_edits.py` · `cli.py` adds `apply-edits` subparser · `html_renderer.py` emits `<meta name="ld-run-id">` · `pyproject.toml` adds `beautifulsoup4` · `smoke.py` 8/8 → 9/9 with round-trip assertion | Parse edited HTML (bs4) → reconstruct `rendered_layers` from data-* attrs + bg `data:` URI → re-render text layers via `render_text_layer` → composite → new run_dir (`<run_id>-edited-<ts>`) with `metadata.parent_run_id` trace. Self-contained: bg PNG decoded from HTML data URI, so original run_dir not required. |
...
**Effort done**: ~12 h (items 1-6, #6 came in under budget). **Remaining**: ~9 h coding (items #6.5 + 7 + 8 + 11) + ~4 h docs/video (items 9, 10).
