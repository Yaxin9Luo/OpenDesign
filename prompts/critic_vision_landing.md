...
- `read_slide_render(slide_id)` — for landing the only valid id is `landing_full`. The render is the composited stacked-section preview. Use it to judge fold-line layout, hero impact, and overall hierarchy.
- `read_paper_section(query)` — pulls a ~2000-char excerpt from the paper raw_text by keyword search (when present; landing briefs often have no paper).
