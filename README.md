...
>
> Product surface unchanged: still 11 tools, still 3-artifact coverage (poster + landing + deck) × full paper / deck / doc ingestion (PDF + DOCX + PPTX + scanned-PDF OCR), still in-browser edit toolbar + `apply-edits` round-trip, still v1.3 interactive landings with CTA / nav / reveal / semantic `<header>/<main>/<footer>`. Smoke **19/19 green** — new check verifies revise-loop preservation + `supersedes_sha256` chain integrity.
>
