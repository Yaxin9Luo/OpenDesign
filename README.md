...
> **Current status** (2026-04-21): **v1.2 paper2any + v1.2.4 / v1.2.5 polish + v1.3.0 / v1.3.1 interactive academic landings** — full **3-artifact coverage** (poster + landing + deck) × full **paper / deck / doc → artifact** ingestion pipeline (PDF + DOCX + PPTX + scanned-PDF OCR), **11 tools wired**, smoke **18/18 green**. Landings now ship with **CTA buttons (per-design-system styling × 6 styles)**, auto-generated top nav, reveal-on-scroll, and semantic `<header>/<main>/<footer>`. **Paper landings pull the paper's own figures + tables** (not NBP stock icons) — real-paper dogfood on longcat-next (editorial style) delivers **0 NBP + 9 paper figures + 1 benchmark table across 8 sections, pass 0.88 in one iteration**. Landing + deck also support **NBP (Gemini 3 Pro Image Preview) imagery** for brand / hero shots. Composite runs **deterministic text-overlap + figure-cross-reference detectors** so geometry + citation errors surface one turn earlier than the critic round.
>
